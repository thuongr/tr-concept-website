    ...
    if (result.class_id && process.env.RESEND_API_KEY) {
      try {
        ...
        const scheduleResponse = await sendClassEmail({
          ...
          to: registration.customer_email,
          ...scheduleEmail,
          attachmentPath: /real work|level 1|business builder|level 2/i.test(registration.selected_program || '') ? undefined : preparation.path,
          attachmentName: /real work|level 1|business builder|level 2/i.test(registration.selected_program || '') ? undefined : preparation.name
        });
